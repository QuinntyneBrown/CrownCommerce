...
public interface IMeetingRepository
{
    ...
    Task<IReadOnlyList<Meeting>> GetByDateRangeAsync(DateTime startUtc, DateTime endUtc, int skip = 0, int take = 50, CancellationToken ct = default);
    Task<IReadOnlyList<Meeting>> GetByEmployeeAsync(Guid employeeId, DateTime startUtc, DateTime endUtc, int skip = 0, int take = 50, CancellationToken ct = default);
