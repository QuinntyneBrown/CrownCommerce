...
import type { HairProduct, GalleryImage } from 'api';
import { HOME_PAGE_CONFIG } from './home-page.config';
import { TestimonialsComponent } from '../../intelligent/testimonials/testimonials';
...
export class HomePage implements OnInit {
  ...
  readonly config = inject(HOME_PAGE_CONFIG);
  ...
  ngOnInit(): void {
    this.loadProducts();
    if (this.config.showGallerySection) {
      this.loadGallery();
    }
  }
  ...
  onNewsletterSubmit(email: string): void {
    ...
    this.newsletterService.subscribe({ email, tags: this.config.newsletterTags }).subscribe({
      next: () => {
  ...
  private loadProducts(): void {
    this.catalogService.getProducts().subscribe({
      next: (products) => {
        this.products.set(products.slice(0, this.config.featuredProductCount));
